...
parser.add_argument('-i','--input', required=True, nargs='+', help="Bam or Sam files to downsample, can use *")
...
parser.add_argument('-s', '--sort', help="if present, sort the bamfile", action='store_true')
...
bamsPaths = list(map(lambda x: os.path.join(os.getcwd(), x), args.input))
...
def filterSam(filePath, primers):
    ...
    def cPrimer(df, key):
        ...
        # if there are multiple reverse and forward primers
        # used to be range(1, len(p[key][0])), but will just be changed to range(0, len(p[key][0]))
        # and do all of the boolean series logic
        statement = True;
        for pNum in range(0, len(p[key][0])):
            statement = statement | df[0].str.contains(p[key][0][pNum], na=False)
        for pNum in range(0, len(p[key][1])):
...
for file in aFiles:
    ...
    if sortBool:
        please = os.system("samtools view -bS {} | samtools sort > {}".format(samFile, bamFile))
    else:
